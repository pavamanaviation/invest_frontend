...
const AdminKYCDetails = () => {
  ...
  // ✅ Secure fetch for signed URL
  // const fetchSecureKycFile = async (customer_id, type) => {
  //   try {
  //     const res = await fetch(`${API_BASE_URL}/api/view-kyc-doc/?customer_id=${customer_id}&type=${type}`);
  ...
  // };
const fetchSecureKycFile = async (customer_id, type) => {
  const admin_id = sessionStorage.getItem("admin_id"); // Assuming you're storing admin_id

  try {
    const res = await fetch(
      `${API_BASE_URL}/api/view-kyc-doc/?customer_id=${customer_id}&type=${type}&admin_id=${admin_id}`
    );
    const data = await res.json();
  ...
  // ✅ On preview file click
  const handleFilePreview = async (customer_id, type) => {
    try {
      const url = await fetchSecureKycFile(customer_id, type);
      setFilePreview(url);
    } catch (error) {
          ...
          {isSearchDone && kycData.length === 0 ? (
            ...
            <tr key={k.customer_id}>
              ...
              <td>
                <span className={`admin-status-badge ${k.pan_status === 1 ? "verified" : "not-verified"}`}>
                  {k.pan_status === 1 ? (
                    <span className="status-icon verified kyc"
                      onClick={() => handleFilePreview(k.customer_id, "pan")}>
                      <FaRegFileAlt />
                    </span>
                  ) : (
                    <span className="status-icon not-verified">
              ...
              <td>
                <span className={`admin-status-badge ${k.aadhar_status === 1 ? "verified" : "not-verified"}`}>
                  {k.aadhar_status === 1 ? (
                    <span className="status-icon verified kyc"
                      onClick={() => handleFilePreview(k.customer_id, "aadhar")}>
                      <FaRegFileAlt />
                    </span>
                  ) : (
                    <span className="status-icon not-verified">
